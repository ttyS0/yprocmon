...
HANDLE hooked_CreateFile(LPCWSTR, DWORD, DWORD, LPSECURITY_ATTRIBUTES, DWORD, DWORD, HANDLE);
...
LONG hooked_RegCreateKeyEx(HKEY, LPCWSTR, DWORD, LPWSTR, DWORD, REGSAM, LPSECURITY_ATTRIBUTES, PHKEY, LPDWORD);
...
int hooked_MessageBox(HWND hWnd, LPCWSTR lpText, LPCWSTR lpCaption, UINT uType) {
    ...
    decltype(MessageBox)* old_MessageBox = (decltype(MessageBox)*) yhook_map[0].original;
    int ret = old_MessageBox(hWnd, lpText, lpCaption, uType);
    
    return ret;
}
HANDLE hooked_CreateFile(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
        
        send_hook_message("CreateFile", {
    ...
    decltype(CreateFile)* old_CreateFile = (decltype(CreateFile)*) yhook_map[1].original;
    HANDLE ret = old_CreateFile(lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);
    
    return ret;
}
WINBOOL hooked_WriteFile(HANDLE hFile, LPCVOID lpBuffer, DWORD nNumberOfBytesToWrite, LPDWORD lpNumberOfBytesWritten, LPOVERLAPPED lpOverlapped) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
        
        send_hook_message("WriteFile", {
    ...
    decltype(WriteFile)* old_WriteFile = (decltype(WriteFile)*) yhook_map[2].original;
    WINBOOL ret = old_WriteFile(hFile, lpBuffer, nNumberOfBytesToWrite, lpNumberOfBytesWritten, lpOverlapped);
    
    return ret;
}
WINBOOL hooked_ReadFile(HANDLE hFile, LPVOID lpBuffer, DWORD nNumberOfBytesToRead, LPDWORD lpNumberOfBytesRead, LPOVERLAPPED lpOverlapped) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
    ...
    decltype(ReadFile)* old_ReadFile = (decltype(ReadFile)*) yhook_map[3].original;
    WINBOOL ret = old_ReadFile(hFile, lpBuffer, nNumberOfBytesToRead, lpNumberOfBytesRead, lpOverlapped);
    
    return ret;
}
HANDLE hooked_HeapCreate(DWORD flOptions, SIZE_T dwInitialSize, SIZE_T dwMaximumSize) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
    ...
    decltype(HeapCreate)* old_HeapCreate = (decltype(HeapCreate)*) yhook_map[4].original;
...
WINBOOL hooked_HeapDestroy(HANDLE hHeap) {
    ...
    if (!entered)
    {
        ...
        send_hook_message("HeapDestroy", {
            {"hHeap", tostring(hHeap)}
        });
        entered = false;
    }
    decltype(HeapDestroy)* old_HeapDestroy = (decltype(HeapDestroy)*) yhook_map[5].original;
...
WINBOOL hooked_HeapFree(HANDLE hHeap, DWORD dwFlags, LPVOID lpMem) {
    ...
    decltype(HeapFree)* old_HeapFree = (decltype(HeapFree)*) yhook_map[6].original;
    WINBOOL ret = old_HeapFree(hHeap, dwFlags, lpMem);
    
    return ret;
}
LONG hooked_RegCreateKeyEx(HKEY hKey, LPCWSTR lpSubKey, DWORD Reserved, LPWSTR lpClass, DWORD dwOptions, REGSAM samDesired, LPSECURITY_ATTRIBUTES lpSecurityAttributes, PHKEY phkResult, LPDWORD lpdwDisposition) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
    ...
    decltype(RegCreateKeyEx)* old_RegCreateKeyEx = (decltype(RegCreateKeyEx)*) yhook_map[7].original;
    LONG ret = old_RegCreateKeyEx(hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired, lpSecurityAttributes, phkResult, lpdwDisposition);
    
    return ret;
}
LONG hooked_RegSetValueEx(HKEY hKey, LPCWSTR lpValueName, DWORD Reserved, DWORD dwType, const BYTE *lpData, DWORD cbData) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
        
        send_hook_message("RegSetValueEx", {
            {"hKey", tostring(hKey)},
    ...
    decltype(RegSetValueEx)* old_RegSetValueEx = (decltype(RegSetValueEx)*) yhook_map[8].original;
    LONG ret = old_RegSetValueEx(hKey, lpValueName, Reserved, dwType, lpData, cbData);
...
LONG hooked_RegDeleteValue(HKEY hKey, LPCWSTR lpValueName) {
    ...
    if (!entered)
    {
        ...
        send_hook_message("RegDeleteValue", {
            {"hKey", tostring(hKey)},
        {"lpValueName", tostring(lpValueName)}
        });
        entered = false;
    }
    decltype(RegDeleteValue)* old_RegDeleteValue = (decltype(RegDeleteValue)*) yhook_map[9].original;
...
LONG hooked_RegCloseKey(HKEY hKey) {
    ...
    if (!entered)
    {
        ...
        send_hook_message("RegCloseKey", {
            {"hKey", tostring(hKey)}
        });
        entered = false;
    }
    decltype(RegCloseKey)* old_RegCloseKey = (decltype(RegCloseKey)*) yhook_map[10].original;
    LONG ret = old_RegCloseKey(hKey);
    
    return ret;
}
LONG hooked_RegOpenKeyEx(HKEY hKey, LPCWSTR lpSubKey, DWORD ulOptions, REGSAM samDesired, PHKEY phkResult) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
        
        send_hook_message("RegOpenKeyEx", {
            {"hKey", tostring(hKey)},
        {"lpSubKey", tostring(lpSubKey)},
    ...
    decltype(RegOpenKeyEx)* old_RegOpenKeyEx = (decltype(RegOpenKeyEx)*) yhook_map[11].original;
    LONG ret = old_RegOpenKeyEx(hKey, lpSubKey, ulOptions, samDesired, phkResult);
...
int hooked_connect(SOCKET s, const sockaddr *name, int namelen) {
    ...
    decltype(connect)* old_connect = (decltype(connect)*) yhook_map[15].original;
...
void * hooked_memcpy(void * __restrict__ __dst, const void * __restrict__ __src, size_t __n) {
    // static bool entered = false;
    if (!entered)
    {
        entered = true;
        
        send_hook_message("memcpy", {
            {"__dst", tostring(__dst)},
